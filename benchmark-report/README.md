...
The library is distributed on PyPI as [`llmd-benchmark-report`](https://pypi.org/project/llmd-benchmark-report/):

```bash
pip install llmd-benchmark-report
```

The canonical import name is `llmd_benchmark_report`. Two aliases exist for backwards compatibility and will be removed in a future release: the bare `benchmark_report` name (shipped in this distribution, used historically by the harness pod scripts) and `llmdbenchmark.analysis.benchmark_report` (a shim in the `llmdbenchmark` package). Both alias the canonical modules with identity preserved.

## Versioning and compatibility

`llmd-benchmark-report` package versions track the Benchmark Report schema version: package `0.2.x` implements BR v0.2.x (and still parses earlier report versions). Within a minor line, releases are strictly additive: new optional fields and new component kinds may be added, but existing fields are never removed, renamed, retyped, or given stricter validation that rejects previously valid reports. A report valid against `0.2.x` validates against every later `0.2.y`. Breaking changes require a new minor line. Packaging-only fixes with no schema change ship as post-releases (e.g. `0.2.1.post1`). Consumers should depend on `llmd-benchmark-report>=0.2.1,<0.3`.
...
See [`br_v0_2_example.yaml`](llmd_benchmark_report/br_v0_2_example.yaml) for a dummy example report (values may be non-sensical). A [JSON Schema](https://json-schema.org/draft/2020-12) for the benchmark report version `0.2` is at [`br_v0_2_json_schema.json`](llmd_benchmark_report/br_v0_2_json_schema.json). 
...
br = llmd_benchmark_report.import_benchmark_report("llmd_benchmark_report/br_v0_2_example.yaml")
...
br = llmd_benchmark_report.BenchmarkReportV02(
    **{
        "run": {
...
```bash
# Print to stdout
benchmark-report -w inference-perf -b 0.2 -s stage_0_session_lifecycle_metrics.json

# Save to file
benchmark-report -w inference-perf -b 0.2 -s stage_0_session_lifecycle_metrics.json benchmark_report_session.yaml
```
